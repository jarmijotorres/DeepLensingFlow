...
training_set =  datanorm(dataset_tensor,pmin = -5.0,pmax = 5.0)
...
for epoch in range(epochs):
    ...
    if (epoch % 100) == 0:
        torch.save(model.state_dict(), output_dir+'DM_weights_normalkappa_checkpoint_%d.pt'%epoch)
    loss_epochs.append(noise_prediction_loss/ batch_idx)
    
print("Finish!!")

torch.save(model.state_dict(), output_dir+'DM_weights_normalkappa_.pt')

epochs_array = np.arange(epochs)
loss_epochs = np.array(loss_epochs)
S = np.array([epochs_array,loss_epochs]).T
np.savetxt(output_dir+'Loss/MAE_loss_epochs.dat',S)
